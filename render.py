...
def render(H, W, K,
    ...
    # 讲精细网络的输出单独拿了出来
    k_extract = ['rgb_map', 'disp_map', 'acc_map', 'weights', 'density_grad', 'input_dirs', 'pred_norm']
...
def render_rays(ray_batch,
    ...
    Returns:
      ...
      z_std: [num_rays]. Standard deviation of distances along ray for each
        sample.
    """
    from nerf_model import RefNeRF
    ...
    if N_importance > 0:
        ...
        # viewdirs 与粗糙网络是相同的
        [raw, pred_norm] = network_query_fn(pts, viewdirs, run_fn).split((4, 3), dim=-1)
        # gpu_tracker.track()
        rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                     pytest=pytest)
        

        density_grad = -RefNeRF.get_grad(raw[..., -1], pts)
        # normal_loss = normal_loss_func(weights, density_grad, pred_norm)
        # print(normal_loss)
        input_dirs = viewdirs[:, None].expand(pts.shape)
        # print(weights.shape, pred_norm.shape, input_dirs.shape, pts.shape)
        
        # bf_loss = bf_loss_func(weights, pred_norm, input_dirs)

    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map,
           'weights': weights, 'density_grad': density_grad, 'input_dirs': input_dirs, 'pred_norm': pred_norm}
