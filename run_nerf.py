...
from nerf_model import RefNeRF, NeRF
...
def create_nerf(args):
    ...
    if args.N_importance > 0:
        model_fine = RefNeRF(D=args.netdepth_fine, W=args.netwidth_fine,
                          input_ch=input_ch, output_ch=4, skips=skips,
                          input_ch_views=3).to(device)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        rgb, disp, acc, weights, density_grad, input_dirs, pred_norm, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays, 
                                                              verbose=i < 10, retraw=True, **render_kwargs_train)
        from nerf_model import WeightedNormalLoss, BackFaceLoss, RefNeRF
        ...
        loss = img_loss + 4e-4 * normal_loss + 0.1 * bf_loss
        # 计算指标
